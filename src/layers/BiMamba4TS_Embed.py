    ...
    def __init__(
        ...
        if self.pos_embed_type is not None:
            self.position_embedding = PositionalEmbedding(
                seq_len, d_model, pos_embed_type, learnable, r_layers, patch_len
            )
...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.pos.dim() > 3:
            ...
            self.pos = torch.reshape(
                self.pos, (-1, self.pos.shape[2], self.pos.shape[3])
            )
            return self.pos
        else:

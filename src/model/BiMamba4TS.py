...
from src.layers.BiMamba4TS_layers import Encoder, EncoderLayer
from src.layers.BiMamba4TS_Embed import PatchEmbedding, TruncateModule
...
class Model(nn.Module):
    def __init__(self, configs, corr=None):
        ...
        if configs.seq_len % configs.stride == 0:
            self.patch_num = int(
                (configs.seq_len - configs.patch_len) / configs.stride + 1
            )
        ...
        self.local_token_layer = PatchEmbedding(
            ...
            process_layer,
            pos_embed_type=(
                None if configs.embed_type in [0, 2] else configs.pos_embed_type
            ),
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        enc_out, _ = self.local_token_layer(
            x_enc.permute(0, 2, 1),

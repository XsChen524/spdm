...
from src.layers.Mamba_EncDec import Encoder, EncoderLayer


# Debug helper: print which Mamba class is used when the model is instantiated
def _log_mamba_version(mamba_class):
    try:
        path = inspect.getfile(mamba_class)
        print(f"[INFO] Using Mamba implementation from: {path}")
    except Exception as e:
        print(f"[INFO] Could not resolve Mamba path: {e}")
            ...
            [
                EncoderLayer(
                    Mamba(
                        ...
                        expand=1,
                        # r_rank=16,  # Block expansion factor)
                    ),
                    Mamba(
                        d_model=configs.d_model,  # Model dimension d_model
                        d_state=configs.d_state,  # SSM state expansion factor
